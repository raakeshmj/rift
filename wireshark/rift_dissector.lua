-- ═══════════════════════════════════════════════════════════════════
-- rift_dissector.lua - Wireshark Lua dissector for RIFT protocol
--
-- Installation:
...
--   3. Or via command line: wireshark -X lua_script:rift_dissector.lua
--
-- The dissector registers on UDP port 9999 by default.
-- ═══════════════════════════════════════════════════════════════════

-- Create the protocol
local rift_proto = Proto("RIFT", "Network Protocol Stack")
...
-- ── Protocol Fields ──────────────────────────────────────────────
local f_version     = ProtoField.uint8("rift.version", "Version", base.DEC)
local f_type        = ProtoField.uint8("rift.type", "Type", base.HEX, pkt_types)
local f_flags       = ProtoField.uint16("rift.flags", "Flags", base.HEX)
local f_seq         = ProtoField.uint32("rift.seq", "Sequence Number", base.DEC)
local f_ack         = ProtoField.uint32("rift.ack", "Acknowledgment Number", base.DEC)
local f_window      = ProtoField.uint16("rift.window", "Window Size", base.DEC)
local f_payload_len = ProtoField.uint16("rift.payload_len", "Payload Length", base.DEC)
local f_ts_send     = ProtoField.uint64("rift.ts_send", "Send Timestamp (us)", base.DEC)
local f_ts_echo     = ProtoField.uint64("rift.ts_echo", "Echo Timestamp (us)", base.DEC)
local f_sack_count  = ProtoField.uint16("rift.sack_count", "SACK Block Count", base.DEC)
local f_conn_id     = ProtoField.uint16("rift.conn_id", "Connection ID", base.DEC)

-- SACK block fields
local f_sack_start  = ProtoField.uint32("rift.sack.start", "SACK Start Seq", base.DEC)
local f_sack_end    = ProtoField.uint32("rift.sack.end", "SACK End Seq", base.DEC)

-- Checksum
local f_checksum    = ProtoField.uint32("rift.checksum", "CRC32 Checksum", base.HEX)

-- Payload
local f_payload     = ProtoField.bytes("rift.payload", "Payload")

-- Flag bits
local f_flag_syn    = ProtoField.bool("rift.flags.syn", "SYN", 16, nil, 0x0001)
local f_flag_fin    = ProtoField.bool("rift.flags.fin", "FIN", 16, nil, 0x0002)
local f_flag_ack    = ProtoField.bool("rift.flags.ack", "ACK", 16, nil, 0x0004)
local f_flag_sack   = ProtoField.bool("rift.flags.sack", "SACK", 16, nil, 0x0008)
local f_flag_nack   = ProtoField.bool("rift.flags.nack", "NACK", 16, nil, 0x0010)
local f_flag_rst    = ProtoField.bool("rift.flags.rst", "RST", 16, nil, 0x0020)
local f_flag_urg    = ProtoField.bool("rift.flags.urg", "URG", 16, nil, 0x0040)
...
function rift_proto.dissector(buffer, pinfo, tree)
    ...
    pinfo.cols.protocol:set("RIFT")

    local subtree = tree:add(rift_proto, buffer(), "RIFT Protocol")
    local offset = 0
...
udp_port:add(9999, rift_proto)

-- Also register on common alternative ports
udp_port:add(9998, rift_proto)  -- Loss simulator proxy
udp_port:add(10000, rift_proto) -- Alternate port
