/* nps_metrics.c — comprehensive metrics & benchmarking tool */
...
/* External NPS APIs */
extern int nps_sender_run_ex(const char *host, uint16_t port,
                             const uint8_t *data, size_t data_len,
                             uint32_t window_size,
                             nps_transfer_result_t *out_result);
extern int nps_receiver_run_ex(uint16_t port, uint8_t *recv_buf,
...
static void run_tcp_benchmark(benchmark_ctx_t *ctx) {
  ...
  size_t total_sent = 0;
  size_t chunk_size = 1400; /* Similar to NPS MSS */
...
static void *nps_receiver_thread(void *arg) {
  benchmark_ctx_t *ctx = (benchmark_ctx_t *)arg;
  uint8_t *buf = malloc(ctx->data_size + 1024 * 1024); /* Overprovision */
  size_t bytes_recv = 0;

  nps_transfer_result_t res = {0};
  int ret = nps_receiver_run_ex(ctx->port, buf, ctx->data_size + 1024 * 1024,
...
static void run_nps_benchmark(benchmark_ctx_t *ctx) {
  pthread_t tid;
  pthread_create(&tid, NULL, nps_receiver_thread, ctx);
  usleep(100000);
  ...
  nps_transfer_result_t res = {0};
  int ret = nps_sender_run_ex("127.0.0.1", ctx->port, data, ctx->data_size,
                              ctx->window_size, &res);
...
int main(int argc, char *argv[]) {
  benchmark_ctx_t ctx = {.mode = "nps",
                         .data_size =
                             100 * 1024 * 1024, /* Default 100 MB for speed */
                         .port = 9000,
                         .window_size = NPS_WINDOW_SIZE,
                         .loss_percent = 0};
  ...
  /* Silence logs */
  nps_log_init(NPS_LOG_ERROR, NULL);

  if (strcmp(ctx.mode, "tcp") == 0) {
    run_tcp_benchmark(&ctx);
  } else {
    run_nps_benchmark(&ctx);
  }
