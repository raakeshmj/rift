/* nps_stress_test.c — congestion stress test with recovery measurement */
...
static void *receiver_thread(void *arg) {
  ...
  uint8_t buf[NPS_MAX_SERIALIZED_SIZE];
  nps_packet_t pkt;
  ...
  while (g_running) {
    ...
    if (nps_packet_deserialize(buf, (size_t)n, &pkt) != 0)
      continue;

    NPS_STAT_INC(&ctx->stats, packets_received);
    NPS_STAT_ADD(&ctx->stats, bytes_received, (uint64_t)n);

    /* Send ACK back */
    nps_packet_t ack;
    nps_packet_build(&ack, NPS_PKT_ACK, NPS_FLAG_ACK, 0, pkt.header.seq_num + 1,
                     NPS_WINDOW_SIZE, 0, NULL, 0, NULL, 0);
    ack.header.ts_send = nps_timestamp_us();
    ack.header.ts_echo = pkt.header.ts_send;

    uint8_t ack_buf[NPS_MAX_SERIALIZED_SIZE];
    int ack_len = nps_packet_serialize(&ack, ack_buf, sizeof(ack_buf));
    if (ack_len > 0) {
...
static double send_burst(stress_ctx_t *ctx, double duration_sec,
                         uint32_t *seq_out) {
  uint64_t start = nps_timestamp_us();
  ...
  while (nps_timestamp_us() < deadline && g_running) {
    /* Simulate loss injection */
    if (ctx->loss_active) {
      double r = (double)rand() / (double)RAND_MAX;
      if (r < ctx->loss_rate) {
        NPS_STAT_INC(&ctx->stats, packets_lost);
        nps_cc_on_loss(&ctx->congestion, false, nps_timestamp_us());
        seq++;
        continue;
      }
    }
    ...
    nps_packet_t pkt;
    nps_packet_build(&pkt, NPS_PKT_DATA, 0, seq, 0, NPS_WINDOW_SIZE, 0, NULL, 0,
                     payload, NPS_MAX_PAYLOAD);
    ...
    if (len > 0) {
      ...
      NPS_STAT_INC(&ctx->stats, packets_sent);
      NPS_STAT_ADD(&ctx->stats, bytes_sent, (uint64_t)len);
      nps_cc_on_ack(&ctx->congestion, 1, nps_timestamp_us());
      sent++;
    }
    ...
    if (sent % (nps_cc_get_cwnd(&ctx->congestion) > 0
                    ? nps_cc_get_cwnd(&ctx->congestion)
                    : 1) ==
        0) {
      usleep(100);
    }
  ...
  uint64_t elapsed = nps_timestamp_us() - start;
...
static void measure_recovery(stress_ctx_t *ctx, double steady_pps,
  ...
  result->cwnd_at_loss = nps_cc_get_cwnd(&ctx->congestion);
  ...
  uint32_t seq = 10000;
  uint64_t loss_start = nps_timestamp_us();
  send_burst(ctx, 2.0, &seq); /* 2 seconds of loss */

  result->cwnd_min = nps_cc_get_cwnd(&ctx->congestion);

  /* Recovery phase */
  ctx->loss_active = false;
  uint64_t recovery_start = nps_timestamp_us();
  ...
  for (int i = 0; i < 20 && g_running; i++) { /* Max 20 × 500ms = 10s */
    ...
    if (current_pps >= target_pps && !recovered) {
      result->recovery_ms =
          (double)(nps_timestamp_us() - recovery_start) / 1000.0;
      result->cwnd_recovered = nps_cc_get_cwnd(&ctx->congestion);
      recovered = true;
      break;
    }
  }
  ...
  uint64_t loss_elapsed = nps_timestamp_us() - loss_start;
  (void)loss_elapsed;
  result->deg_pct =
...
static void print_results(stress_ctx_t *ctx) {
  printf("\n");
  printf("╔═══════════════════════════════════════════════════════════╗\n");
  printf("║           NPS Congestion Stress Test Results             ║\n");
  printf("╠═══════════════════════════════════════════════════════════╣\n");
...
int main(int argc, char *argv[]) {
  uint16_t port = NPS_DEFAULT_PORT + 10; /* Avoid conflict */
  int opt;
  ...
  while ((opt = getopt(argc, argv, "p:h")) != -1) {
    ...
    case 'p':
      ...
      break;
    case 'h':
      printf("NPS Congestion Stress Test\n");
  ...
  nps_crc32_init();
  nps_log_init(NPS_LOG_INFO, NULL);
  nps_trace_init("/tmp/nps_stress_trace", NPS_TRACE_FMT_BOTH);
  srand((unsigned)nps_timestamp_us());
  ...
  nps_cc_init(&ctx.congestion);
  nps_rtt_init(&ctx.rtt);
  nps_stats_init(&ctx.stats);
  ...
  NPS_LOG_INFO("╔═══════════════════════════════════════════════════╗");
  NPS_LOG_INFO("║          NPS Congestion Stress Test              ║");
  NPS_LOG_INFO("╚═══════════════════════════════════════════════════╝");

  /* ── Phase 1: Ramp Up ─────────────────────────────────────────── */
  NPS_LOG_INFO("Phase 1: Ramp up (3 seconds)...");
  uint32_t seq = 1;
  double ramp_pps = send_burst(&ctx, 3.0, &seq);
  ctx.results[0].steady_pps = ramp_pps;
  NPS_LOG_INFO("  Steady-state PPS: %.0f", ramp_pps);

  /* ── Phase 2: Saturate ────────────────────────────────────────── */
  NPS_LOG_INFO("Phase 2: Saturate (5 seconds)...");
  ...
  for (uint32_t burst = 0; burst < ctx.burst_count && g_running; burst++) {
    ...
    if (burst + 1 < ctx.burst_count) {
      NPS_LOG_INFO("  Cooldown (2 seconds)...");
  ...
  print_results(&ctx);
  nps_stats_report(&ctx.stats);
  nps_trace_dump_summary();
