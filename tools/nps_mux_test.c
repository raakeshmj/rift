/* nps_mux_test.c — multiplexed stream test (4 × 256 KB concurrent) */
...
static void *mux_recv_thread(void *arg) {
  ...
  while (*rta->running) {
    ...
    nps_mux_dispatch(rta->mux, &pkt, &from, from_len);
  }
...
static void *mux_send_thread(void *arg) {
  ...
  while (*sta->running) {
    int sent = nps_mux_transmit_all(sta->mux);
    if (sent == 0)
...
int main(void) {
  ...
  printf("\n╔═══════════════════════════════════════════════════════╗\n");
  printf("║       NPS Connection Multiplexing Test              ║\n");
  printf("╠═══════════════════════════════════════════════════════╣\n");
  printf("║  Streams:   %d                                       ║\n",
         NUM_STREAMS);
  ...
  nps_mux_t sender_mux, receiver_mux;
  nps_mux_init(&sender_mux, send_fd);
  nps_mux_init(&receiver_mux, recv_fd);
  ...
  for (int i = 0; i < NUM_STREAMS; i++) {
    ...
    nps_stream_t *ss = nps_mux_create_stream(&sender_mux, cid);
    if (!ss) {
      NPS_LOG_ERROR("Failed to create sender stream %u", cid);
      return 1;
    }
    nps_mux_send(&sender_mux, cid, send_data[i], STREAM_DATA_SIZE);

    nps_stream_t *rs = nps_mux_create_stream(&receiver_mux, cid);
    if (!rs) {
      NPS_LOG_ERROR("Failed to create receiver stream %u", cid);
      return 1;
    }
  ...
  while (nps_timestamp_us() - start < timeout_us) {
    bool all_done = true;
    for (int i = 0; i < NUM_STREAMS; i++) {
      nps_stream_t *s = nps_mux_find_stream(&sender_mux, (uint16_t)(100 + i));
  ...
  for (int i = 0; i < NUM_STREAMS; i++) {
    ...
    nps_stream_t *ss = nps_mux_find_stream(&sender_mux, cid);
    nps_stream_t *rs = nps_mux_find_stream(&receiver_mux, cid);

    uint64_t pkts_sent = ss ? NPS_STAT_GET(&ss->stats, packets_sent) : 0;
    uint64_t pkts_recv = rs ? NPS_STAT_GET(&rs->stats, packets_received) : 0;
    ...
    double stream_mbps =
        (double)(pkts_sent * NPS_MAX_PAYLOAD * 8) / (elapsed_sec * 1000000.0);
    ...
    if (match && recv_bytes > 0)
      total_pass++;
    total_bytes_sent += pkts_sent * NPS_MAX_PAYLOAD;
  }
  ...
  /* Cleanup */
  nps_mux_shutdown(&sender_mux);
  nps_mux_shutdown(&receiver_mux);
  close(send_fd);
  close(recv_fd);
  nps_log_shutdown();
