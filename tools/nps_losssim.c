/* nps_losssim.c — UDP loss simulator proxy */
...
static void print_usage(const char *prog) {
  printf("NPS Packet Loss Simulator\n\n");
...
int main(int argc, char *argv[]) {
  ...
  NPS_LOG_INFO("╔═══════════════════════════════════════════════╗");
  NPS_LOG_INFO("║      NPS Packet Loss Simulator               ║");
  NPS_LOG_INFO("╠═══════════════════════════════════════════════╣");
  NPS_LOG_INFO("║  Listen:   :%d → :%d                        ║",
               sim.listen_port, sim.target_port);
  NPS_LOG_INFO("║  Loss:     %.1f%%                              ║",
               sim.loss_rate * 100.0);
  NPS_LOG_INFO("║  Delay:    %.1f ms ± %.1f ms                  ║",
  ...
  uint8_t buf[NPS_MAX_SERIALIZED_SIZE + 256];
  ...
  while (g_running) {
    ...
    if (drop) {
      sim.total_dropped++;
      NPS_LOG_DEBUG("DROPPED packet (%s, %zd bytes)",
    ...
    NPS_LOG_TRACE("Forwarded %zd bytes (%s)", n,
                  is_from_client ? "C→S" : "S→C");
  }

  NPS_LOG_INFO("═══════════════════════════════════════════════");
  NPS_LOG_INFO("  Loss Simulator Statistics");
  NPS_LOG_INFO("  Forwarded:   %" PRIu64 " packets", sim.total_forwarded);
  NPS_LOG_INFO("  Dropped:     %" PRIu64 " packets", sim.total_dropped);
  NPS_LOG_INFO("  C→S:         %" PRIu64, sim.client_to_server);
  NPS_LOG_INFO("  S→C:         %" PRIu64, sim.server_to_client);
  NPS_LOG_INFO("  Total bytes: %" PRIu64, sim.total_bytes);
  double actual_loss =
      sim.total_forwarded + sim.total_dropped > 0
          ? (double)sim.total_dropped /
                (double)(sim.total_forwarded + sim.total_dropped) * 100.0
          : 0.0;
  NPS_LOG_INFO("  Actual loss: %.2f%%", actual_loss);
  NPS_LOG_INFO("═══════════════════════════════════════════════");
