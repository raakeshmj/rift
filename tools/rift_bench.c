/* rift_bench.c — throughput/latency benchmarking tool */
...
static void *receiver_thread(void *arg) {
  ...
  size_t bytes_recv = 0;
  a->start_us = rift_timestamp_us();
  a->result =
      rift_receiver_run(a->port, buf, buf_size, &bytes_recv, a->window_size);
  a->end_us = rift_timestamp_us();
...
static void *sender_thread(void *arg) {
  ...
  a->start_us = rift_timestamp_us();
  a->result =
      rift_sender_run("127.0.0.1", a->port, data, a->data_size, a->window_size);
  a->end_us = rift_timestamp_us();
...
static void run_benchmark(size_t data_size, uint32_t window_size, uint16_t port,
  ...
  if (send_args.result == 0) {
    ...
    printf("  Packets:     ~%zu\n", data_size / RIFT_MAX_PAYLOAD + 1);
    printf("  PPS:         ~%.0f\n",
           (double)(data_size / RIFT_MAX_PAYLOAD + 1) / elapsed_s);
  } else {
...
int main(int argc, char *argv[]) {
  ...
  int runs = 3;
  rift_log_level_t log_level = RIFT_LOG_WARN; /* Quiet by default for bench */
