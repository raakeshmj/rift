...
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  RIFT Comprehensive Metrics Suite${NC}"
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
...
# 2. Run Benchmarks (RIFT vs TCP)
echo -e "\n${BLUE}[2/5] Running Throughput & Latency Benchmarks (1GB)...${NC}"

# Run RIFT baseline (1GB)
RIFT_JSON=$(./build/rift_metrics --mode rift --size 1073741824)
RIFT_TP=$(echo $RIFT_JSON | grep -o 'throughput_mbps": [0-9.]*' | awk '{print $2}')
RIFT_RTT=$(echo $RIFT_JSON | grep -o 'avg_rtt_us": [0-9]*' | awk '{print $2}')
RIFT_MEM=$(echo $RIFT_JSON | grep -o 'memory_usage_kb": [0-9]*' | awk '{print $2}')
RIFT_RETX=$(echo $RIFT_JSON | grep -o 'retransmission_overhead": [0-9.]*' | awk '{print $2}')

# Run TCP baseline
TCP_JSON=$(./build/rift_metrics --mode tcp --size 104857600)
TCP_TP=$(echo $TCP_JSON | grep -o 'throughput_mbps": [0-9.]*' | awk '{print $2}')

# Calculate improvement/overhead
TP_DIFF=$(echo "scale=2; ($RIFT_TP - $TCP_TP) / $TCP_TP * 100" | bc)
MEM_MB=$(echo "scale=1; $RIFT_MEM / 1024" | bc)

echo -e "  RIFT Throughput: ${GREEN}${RIFT_TP} Mbps${NC}"
echo -e "  TCP Throughput: ${GREEN}${TCP_TP} Mbps${NC} (Diff: ${TP_DIFF}%)"
echo -e "  RIFT Avg RTT:    ${GREEN}${RIFT_RTT} µs${NC}"
echo -e "  Retransmissions: ${GREEN}${RIFT_RETX}%${NC}"
...
./build/rift_losssim 9000 9001 10 &
SIM_PID=$!
sleep 1

# Run benchmark against loss simulator port (9000 -> 9001)
# Note: This requires rift_metrics to target port 9000 (simulator input)
# Ideally we'd modify rift_metrics args, assuming generic port for now
LOSS_JSON=$(./build/rift_metrics --mode rift --size 10485760 --port 9000 --loss 10 2>/dev/null || echo "failed")
...
if command -v valgrind &> /dev/null; then
  VALGRIND_OUT=$(valgrind --leak-check=full --error-exitcode=1 ./build/rift_metrics --size 102400 2>&1)
  if [ $? -eq 0 ]; then
...
if [[ "$(uname -s)" == "Linux" ]]; then
  ...
  PPS=$(echo "scale=0; $RIFT_TP * 1000000 / 1500 / 8" | bc)
...
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
echo "1. Throughput:        ${RIFT_TP} Mbps (vs TCP ${TCP_TP} Mbps)"
echo "2. Latency (RTT):     ${RIFT_RTT} µs (loopback)"
