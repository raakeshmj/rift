/* nps_congestion.c — TCP Cubic congestion control */
...
void nps_cc_init(nps_congestion_t *cc) {
  cc->cwnd = NPS_CWND_INIT;
  cc->ssthresh = NPS_SLOW_START_THRESH;
  cc->phase = NPS_CC_SLOW_START;
  cc->w_max = 0.0;
  cc->k = 0.0;
  cc->epoch_start_us = 0;
  cc->acked_bytes = 0;
  cc->lost_count = 0;
  cc->last_loss_us = 0;
  cc->max_cwnd = NPS_CWND_INIT;
  cc->total_losses = 0;
  cc->total_fast_recoveries = 0;
}

static void cubic_reset_epoch(nps_congestion_t *cc) { cc->epoch_start_us = 0; }

static double cubic_w(nps_congestion_t *cc, uint64_t now_us) {
  if (cc->epoch_start_us == 0) {
    cc->epoch_start_us = now_us;
    /* K = cbrt(W_max * beta / C) */
    if (cc->w_max > 0) {
      cc->k = cbrt(cc->w_max * NPS_CUBIC_BETA / NPS_CUBIC_C);
    } else {
      cc->k = 0.0;
    }
  }
  ...
  double dt = t - cc->k;
  double w_cubic = NPS_CUBIC_C * dt * dt * dt + cc->w_max;
...
void nps_cc_on_ack(nps_congestion_t *cc, uint32_t acked, uint64_t now_us) {
  ...
  switch (cc->phase) {
  case NPS_CC_SLOW_START:
    /* Exponential growth: increase cwnd by 1 for each ACK */
    cc->cwnd += acked;
    if (cc->cwnd >= cc->ssthresh) {
      cc->phase = NPS_CC_CONGESTION_AVOIDANCE;
      cubic_reset_epoch(cc);
    }
    break;

  case NPS_CC_CONGESTION_AVOIDANCE: {
    ...
    double w_tcp =
        cc->w_max * (1.0 - NPS_CUBIC_BETA) + (double)acked / cc->cwnd;
...
void nps_cc_on_loss(nps_congestion_t *cc, bool is_timeout, uint64_t now_us) {
  ...
  if (is_timeout) {
    ...
    cc->ssthresh = cc->cwnd * NPS_CUBIC_BETA;
    if (cc->ssthresh < 2.0)
      cc->ssthresh = 2.0;
    cc->cwnd = NPS_CWND_INIT;
    cc->phase = NPS_CC_SLOW_START;
