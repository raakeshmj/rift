/* rift_crypto.c — ChaCha20 stream cipher (RFC 8439) */
...
void rift_crypto_init(rift_crypto_ctx_t *ctx, const uint8_t *key) {
  ...
  if (key) {
    memcpy(ctx->key, key, RIFT_CHACHA20_KEY_SIZE);
    ctx->enabled = true;
  }
}
...
int rift_crypto_encrypt(const rift_crypto_ctx_t *ctx, uint8_t *data, size_t len,
  ...
  uint8_t nonce[RIFT_CHACHA20_NONCE_SIZE];
  rift_crypto_derive_nonce(conn_id, seq_num, nonce);
...
int rift_crypto_generate_key(uint8_t key_out[RIFT_CHACHA20_KEY_SIZE]) {
  ...
  ssize_t n = read(fd, key_out, RIFT_CHACHA20_KEY_SIZE);
  close(fd);

  return (n == RIFT_CHACHA20_KEY_SIZE) ? 0 : -1;
}

int rift_crypto_parse_hex_key(const char *hex,
                             uint8_t key[RIFT_CHACHA20_KEY_SIZE]) {
  size_t hex_len = strlen(hex);
  if (hex_len != RIFT_CHACHA20_KEY_SIZE * 2)
    return -1;

  for (int i = 0; i < RIFT_CHACHA20_KEY_SIZE; i++) {
    unsigned int byte;
