/* nps_protocol.c — serialize/deserialize, timestamps, packet builder */
...
int nps_packet_serialize(const nps_packet_t *pkt, uint8_t *buf,
                         size_t buf_len) {
  uint16_t sack_count = pkt->header.sack_count;
  if (sack_count > NPS_MAX_SACK_BLOCKS)
    sack_count = NPS_MAX_SACK_BLOCKS;

  uint16_t payload_len = pkt->header.payload_len;
  if (payload_len > NPS_MAX_PAYLOAD)
    return -1;

  size_t sack_size = sack_count * sizeof(nps_sack_block_t);
  size_t total = NPS_HEADER_SIZE + sack_size + 4 /* CRC32 */ + payload_len;
  ...
  *p++ = pkt->header.type;
  ...
  crc = nps_crc32_update(crc, buf, (size_t)(crc_pos - buf));
  /* CRC over payload */
  if (payload_len > 0) {
    crc = nps_crc32_update(crc, crc_pos + 4, payload_len);
  }
...
int nps_packet_deserialize(const uint8_t *buf, size_t buf_len,
                           nps_packet_t *pkt) {
  if (buf_len < NPS_HEADER_SIZE + 4) /* Minimum: header + CRC */
    return -1;
  ...
  /* Validate version */
  if (pkt->header.version != NPS_VERSION)
    return -1;

  /* Validate SACK count */
  if (pkt->header.sack_count > NPS_MAX_SACK_BLOCKS)
    return -1;

  /* Validate payload length */
  if (pkt->header.payload_len > NPS_MAX_PAYLOAD)
    return -1;

  size_t sack_size = pkt->header.sack_count * sizeof(nps_sack_block_t);
  size_t expected = NPS_HEADER_SIZE + sack_size + 4 + pkt->header.payload_len;
  if (buf_len < expected)
    return -1;
  ...
  crc = nps_crc32_update(crc, buf, (size_t)(crc_pos - buf));
  if (pkt->header.payload_len > 0) {
    crc = nps_crc32_update(crc, crc_pos + 4, pkt->header.payload_len);
  }
  crc = nps_crc32_finalize(crc);
...
void nps_packet_build(nps_packet_t *pkt, uint8_t type, uint16_t flags,
  ...
  pkt->header.version = NPS_VERSION;
  ...
  if (sack_blocks && sack_count > 0) {
    ...
    memcpy(pkt->sack_blocks, sack_blocks,
           sack_count * sizeof(nps_sack_block_t));
  }

  if (payload && payload_len > 0) {
    if (payload_len > NPS_MAX_PAYLOAD)
      payload_len = NPS_MAX_PAYLOAD;
    pkt->header.payload_len = payload_len;
...
const char *nps_pkt_type_str(uint8_t type) {
  ...
  case NPS_PKT_SYN:
    return "SYN";
  case NPS_PKT_SYN_ACK:
    return "SYN_ACK";
  case NPS_PKT_FIN:
    return "FIN";
  case NPS_PKT_FIN_ACK:
    return "FIN_ACK";
  case NPS_PKT_RST:
    return "RST";
  case NPS_PKT_PING:
...
const char *nps_conn_state_str(nps_conn_state_t state) {
  switch (state) {
  case NPS_STATE_CLOSED:
    return "CLOSED";
  case NPS_STATE_LISTEN:
    return "LISTEN";
  case NPS_STATE_SYN_SENT:
    return "SYN_SENT";
  case NPS_STATE_SYN_RCVD:
    return "SYN_RCVD";
  case NPS_STATE_ESTABLISHED:
    return "ESTABLISHED";
  case NPS_STATE_FIN_WAIT:
    return "FIN_WAIT";
  case NPS_STATE_CLOSE_WAIT:
    return "CLOSE_WAIT";
  case NPS_STATE_LAST_ACK:
    return "LAST_ACK";
  case NPS_STATE_TIME_WAIT:
    return "TIME_WAIT";
  default:
    return "UNKNOWN";
  }
