/* rift_trace.c — packet trace logger (JSON Lines + columnar text) */
...
static struct {
  FILE *json_fp;
  FILE *text_fp;
  rift_trace_format_t format;
  rift_trace_stats_t stats;
...
static const char *event_names[] = {
    [RIFT_TRACE_TX] = "TX",           [RIFT_TRACE_RX] = "RX",
    [RIFT_TRACE_ACK] = "ACK",         [RIFT_TRACE_SACK] = "SACK",
    [RIFT_TRACE_RETX] = "RETX",       [RIFT_TRACE_DROP] = "DROP",
    [RIFT_TRACE_CWND] = "CWND",       [RIFT_TRACE_RTT] = "RTT",
    [RIFT_TRACE_STATE] = "STATE",     [RIFT_TRACE_LOSS] = "LOSS",
    [RIFT_TRACE_TIMEOUT] = "TIMEOUT",
};

static const char *event_colors[] = {
    [RIFT_TRACE_TX] = "\033[32m",      /* Green    */
    [RIFT_TRACE_RX] = "\033[36m",      /* Cyan     */
    [RIFT_TRACE_ACK] = "\033[34m",     /* Blue     */
    [RIFT_TRACE_SACK] = "\033[35m",    /* Magenta  */
    [RIFT_TRACE_RETX] = "\033[33m",    /* Yellow   */
    [RIFT_TRACE_DROP] = "\033[31m",    /* Red      */
    [RIFT_TRACE_CWND] = "\033[37m",    /* White    */
    [RIFT_TRACE_RTT] = "\033[37m",     /* White    */
    [RIFT_TRACE_STATE] = "\033[1m",    /* Bold     */
    [RIFT_TRACE_LOSS] = "\033[31m",    /* Red      */
    [RIFT_TRACE_TIMEOUT] = "\033[33m", /* Yellow */
};

#define COLOR_RESET "\033[0m"

const char *rift_trace_event_str(rift_trace_event_t event) {
  if (event <= RIFT_TRACE_TIMEOUT)
...
int rift_trace_init(const char *base_path, rift_trace_format_t format) {
  ...
  g_trace.base_ts = rift_timestamp_us();

  if (base_path) {
    char path[512];

    if (format & RIFT_TRACE_FMT_JSON) {
      snprintf(path, sizeof(path), "%s.jsonl", base_path);
      g_trace.json_fp = fopen(path, "w");
      if (!g_trace.json_fp) {
        RIFT_LOG_ERROR("Failed to open trace file: %s", path);
        return -1;
      }
      RIFT_LOG_INFO("Trace JSON output: %s", path);
    }

    if (format & RIFT_TRACE_FMT_TEXT) {
      ...
      fprintf(g_trace.text_fp,
              ...
              "────────── ────── ────── ────── ──────── ──────── "
              "────────────────────\n");
      RIFT_LOG_INFO("Trace TEXT output: %s", path);
    }
  }
...
static void trace_write_json(const rift_trace_entry_t *e) {
  if (!g_trace.json_fp)
    return;
  ...
  fprintf(g_trace.json_fp,
          ...
          "\"rtt_ms\":%.3f,\"rto_ms\":%.3f",
          rel_ms, e->direction, rift_trace_event_str(e->event),
          rift_pkt_type_str(e->pkt_type), e->flags, e->seq_num, e->ack_num,
          e->payload_len, e->window_size, e->conn_id, e->cwnd, e->ssthresh,
          e->rtt_ms, e->rto_ms);

  if (e->event == RIFT_TRACE_STATE) {
    fprintf(g_trace.json_fp, ",\"old_state\":\"%s\",\"new_state\":\"%s\"",
            rift_conn_state_str((rift_conn_state_t)e->old_state),
            rift_conn_state_str((rift_conn_state_t)e->new_state));
  }
...
static void trace_write_text(const rift_trace_entry_t *e) {
  if (!g_trace.text_fp)
    return;
  ...
  /* Detail or SACK info */
  char detail_buf[128] = "";
  if (e->event == RIFT_TRACE_SACK && e->sack_count > 0) {
    int dpos = 0;
    dpos +=
    ...
    snprintf(detail_buf + dpos, sizeof(detail_buf) - (size_t)dpos, "]");
  } else if (e->event == RIFT_TRACE_STATE) {
  ...
  fprintf(g_trace.text_fp,
          ...
          rift_trace_event_str(e->event), rift_pkt_type_str(e->pkt_type),
...
static void trace_update_stats(const rift_trace_entry_t *e) {
  rift_trace_stats_t *s = &g_trace.stats;
  ...
  case RIFT_TRACE_ACK:
    s->count_ack++;
    break;
  case RIFT_TRACE_SACK:
    s->count_sack++;
    break;
  case RIFT_TRACE_RETX:
    s->count_retx++;
    break;
  case RIFT_TRACE_DROP:
    s->count_drop++;
    break;
  case RIFT_TRACE_CWND:
    s->count_cwnd++;
    break;
  case RIFT_TRACE_RTT:
    s->count_rtt++;
    break;
  case RIFT_TRACE_STATE:
    s->count_state++;
    break;
  case RIFT_TRACE_LOSS:
    s->count_loss++;
    break;
  case RIFT_TRACE_TIMEOUT:
    s->count_timeout++;
    break;
  }
...
void rift_trace_dump_summary(void) {
  ...
  RIFT_LOG_INFO("╔═══════════════════ Trace Summary ═══════════════════╗");
  RIFT_LOG_INFO("║  Duration:     %.2f ms", dur_ms);
  RIFT_LOG_INFO("║  Total events: %" PRIu64, s.total_events);
  RIFT_LOG_INFO("║  ── Breakdown ──");
  RIFT_LOG_INFO("║  TX:       %" PRIu64, s.count_tx);
  RIFT_LOG_INFO("║  RX:       %" PRIu64, s.count_rx);
  RIFT_LOG_INFO("║  ACK:      %" PRIu64, s.count_ack);
  RIFT_LOG_INFO("║  SACK:     %" PRIu64, s.count_sack);
  RIFT_LOG_INFO("║  RETX:     %" PRIu64, s.count_retx);
  RIFT_LOG_INFO("║  DROP:     %" PRIu64, s.count_drop);
  RIFT_LOG_INFO("║  CWND chg: %" PRIu64, s.count_cwnd);
  RIFT_LOG_INFO("║  RTT meas: %" PRIu64, s.count_rtt);
  RIFT_LOG_INFO("║  STATE:    %" PRIu64, s.count_state);
  RIFT_LOG_INFO("║  LOSS det: %" PRIu64, s.count_loss);
  RIFT_LOG_INFO("║  TIMEOUT:  %" PRIu64, s.count_timeout);
  if (s.count_tx > 0) {
    double retx_pct = (double)s.count_retx / (double)s.count_tx * 100.0;
    RIFT_LOG_INFO("║  Retransmit ratio: %.2f%%", retx_pct);
  }
  RIFT_LOG_INFO("╚════════════════════════════════════════════════════╝");
}
